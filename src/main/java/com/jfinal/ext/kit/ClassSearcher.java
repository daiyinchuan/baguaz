...
public class ClassSearcher {
    ...
    public <T> List<Class<? extends T>> search() {
        ...
        if(LOG.isDebugEnabled()){
	        if(classFileList.size()>0){
	        	LOG.debug("在类路径"+classpath+"中找到"+classFileList.size()+"个文件：");
	        	classFileList.stream().limit(8).forEach(c->LOG.debug(c));
	        }else{
	        	LOG.debug("在类路径"+classpath+"中没找到任何匹配类文件");
	        }
        }
        List<String> classFileListInJar =findjarFiles(libDir);
        if(LOG.isDebugEnabled()){
	        if(classFileListInJar.size()>0){
	        	LOG.debug("在"+libDir+(includeAllJarsInLib?"":" "+this.includeJars)+"中找到"+classFileListInJar.size()+"个文件：");
	        	classFileListInJar.stream().limit(8).forEach(c->LOG.debug(c));
	        }else{
	        	LOG.debug("在"+libDir+"中没找到任何匹配类文件");
	        }
        }
        classFileList.addAll(classFileListInJar);
    ...
    private List<String> findjarFiles(String baseDirName) {
        ...
        if (!baseDir.exists() || !baseDir.isDirectory()) {
        	...
        	LOG.debug("includeJars="+includeJars);
